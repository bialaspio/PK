...
//--------------------------------------------------------------------------------------
/*var routingControl_G;

function wyznacz_trase_G(callback) {
	// Usuń starą trasę
	if (routingControl_G !== undefined) {
	  map.removeControl(routingControl_G);
	}
  
	// Utwórz nowy kontroler trasy
	routingControl_G = L.Routing.control({
	  waypoints: [
		L.latLng(x_start_g, y_start_g),
		L.latLng(x_end_g, y_end_g)
	  ],
	  lineOptions: {
		styles: [{ color: 'orangered', opacity: 0.7, weight: 5 }]
	  },
	  containerClassName: 'Trasa_G',
	  language: 'pl',
	  routeWhileDragging: true,
	  showAlternatives: false,
	  showRouteInstructions: true // Włącz opis trasy
	}).addTo(map);

	// Dopasowanie widoku mapy do trasy
	const bounds = L.latLngBounds([L.latLng(x_start_g, y_start_g), L.latLng(x_end_g, y_end_g)]);
	map.fitBounds(bounds);

	//get the route
	routingControl_G.on('routesfound', function(e) {
		var routes = e.routes;
		var summary = routes[0].summary;
		console.log({summary});
		// get middle of route
		var route = e.routes[0];
		var middle = route.coordinates[Math.floor(route.coordinates.length / 2)];
		console.log({middle});
		
		let distance = summary.totalDistance / 1000; // km
		let time = summary.totalTime;
		console.log({time});

		// Przeliczamy czas w sekundach na godziny i minuty
		...
		var tooltipContent = `
			<div style="display: grid;">
				<div style="display: inline; height: 18px;">
					<img src="./legenda/car20x20.jpg">`;
					if (hours < 1) {
						//console.log(`Czas: ${Math.round(time)} min`);
						tooltipContent+=`<text class="tooltip_czas">${minutes} min</text>`;
					} else {
						tooltipContent+=`<text class="tooltip_czas">${hours} godz ${minutes} min</text>`;
					}
					tooltipContent+=`</div>
				<div style="display: inline; height: 13px;">
					<text class="tooltip_km">${distance} km</text>
				</div>
			</div>`;


		// add tooltip on middle of route
		L.popup({
			latlng: middle,
			autoPan: true,
			closeOnClick: false,
...
*/
//**********************************************************************
// Użycie Api google miejsca i routnig 
//**********************************************************************
/*
// Inicjalizacja biblioteki jQuery UI Autocomplete
$(function() {
	var input = $("#punktStartTrasa_googleAPI");
	console.log({input});
  
	// Wybór pola, dla którego ma być używane autouzupełnianie
	wyszukajPo3LiterachGAPI(input,'start_GAPI');
...
// Inicjalizacja biblioteki jQuery UI Autocomplete
$(function() {
	var input = $("#punktEndTrasa_googleAPI");
	console.log({input});
  
	// Wybór pola, dla którego ma być używane autouzupełnianie
	wyszukajPo3LiterachGAPI(input,'end_GAPI');
...
function wyszukajPo3LiterachGAPI(input,rodz_mark) {
		...
		var geocoder = new google.maps.Geocoder();
	  	...
	  	geocoder.geocode({ address: address }, function(results, status) {
			if (status === 'OK') {
				...
				var fadress = results[0].formatted_address;
				var location = results[0].geometry.location;
				var lat = location.lat();
				var lng = location.lng();
				
				if (rodz_mark === 'end_GAPI'){ 
					map.eachLayer(function(layer) {
						if (layer instanceof L.Marker && layer.options.title === "end_GAPI") {
						map.removeLayer(layer);
						}
					});
				}
					...
					map.eachLayer(function(layer) {
						if (layer instanceof L.Marker && layer.options.title === "start_GAPI") {
						map.removeLayer(layer);
						}
					});
				}
				...
				// Wyświetl nowy znacznik
				//var marker = L.marker([lat, lng]).addTo(map);
				if (rodz_mark === 'start_GAPI'){

					m_start_gapi = fadress;

					var marker = L.marker([lat, lng], {
						title: 'start_GAPI' // nazwa wyświetlana w popupie
					  }).addTo(map);

					// Ustaw widok mapy na nową lokalizację
					map.setView([lat, lng],17);

					if (m_start_gapi > 0 &&  m_end_gapi > 0){
						document.addEventListener('keydown', function(event) { // Użyj `document.addEventListener` do wykrycia naciśnięcia klawisza
							if (event.key === 'Enter' && event) { // Sprawdź, czy event istnieje przed użyciem jego właściwości
							  wyznacz_trase_GAPI(m_start_gapi, m_end_gapi);
							}
						});
					}
					...
					var marker = L.marker([lat, lng], {
						title: 'end_GAPI' // nazwa wyświetlana w popupie
					}).addTo(map);
					// Ustaw widok mapy na nową lokalizację
					map.setView([lat, lng],17);
					...
					if (m_start_gapi.length > 0 &&  m_end_gapi.length > 0){
						document.addEventListener('keydown', function(event) { // Użyj `document.addEventListener` do wykrycia naciśnięcia klawisza
							if (event.key === 'Enter' && event) { // Sprawdź, czy event istnieje przed użyciem jego właściwości
							  wyznacz_trase_GAPI(m_start_gapi, m_end_gapi);
							}
				...
				toggleLayersGroup(buttonPRG,labelsPRG);
				toggleLayersGroup(buttonOBSAK,labelsOBSAK);
				toggleLayersGroup(buttonGEODEZJA,labelsGEODEZJA);
...
function wyznacz_trase_GAPI(startPoint, endPoint) {
	...
	function(response, status) {
		...
		if (status === google.maps.DirectionsStatus.OK) {
			...
			// Rysowanie linii trasy na mapie
			...
			// Wyświetlenie danych o odległości i czasie przejazdu w tooltipie
			...
			// Dodanie tooltipa z tekstem "Trasa"
			polyline.bindTooltip(tooltipContent).openTooltip();
